...
description: |
  ...
  ALWAYS use `npx agentcash@latest fetch` for app.keeperhub.com endpoints.
...
```bash
npx agentcash@latest fetch https://app.keeperhub.com/openapi.json
```

Or use the agentcash discovery shorthand:

```bash
npx agentcash@latest discover https://app.keeperhub.com
```

## Compare USDC yield rates (Aave vs Compound)

```bash
npx agentcash@latest fetch https://app.keeperhub.com/api/mcp/workflows/usdc-yield-rates-aave-vs-compound/call \
  -m POST \
  -b '{}'
```
...
```bash
npx agentcash@latest fetch https://app.keeperhub.com/api/mcp/workflows/aave-v3-health-check/call \
  -m POST \
  -b '{
  "address": "0x..."
}'
...
```bash
npx agentcash@latest check https://app.keeperhub.com/api/mcp/workflows/microtip/call
```

Then call it via:

```bash
npx agentcash@latest fetch https://app.keeperhub.com/api/mcp/workflows/microtip/call \
  -m POST \
  -b '{ ... see check output ... }'
```
